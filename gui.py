...
titleLabel = tk.Label(window, text="BEAT THE ALGORITHM",
...
comCanvas1_container = comCanvas3.create_image(
    10, 10, anchor=tk.NW, image=blankImage)
...
emoLabel = tk.Label(attrFrame, text="Emotion: N/A",
                    font=('Arial 18'), bd=1, relief="solid")
emoLabel.grid(row=0, column=0, sticky="W", pady=10)
# titleLabel.pack(side=tk.LEFT, ipadx=10, ipady=10)

secondLabel = tk.Label(attrFrame, text="Smt: N/A",
                       font=('Arial 18'), bd=1, relief="solid")
...
def setImagesAndAttr():
    ...
    os.remove(
        "C:/Users/admokhta/Documents/InternalProject/BeatTheAlgorithm/images/representations_vgg_face.pkl")
    ...
    # print(example_list)
    # result = [x.strip() for x in csv_results.split(',')]
    # chunks = result.split('\n')
    # print(chunks)
...
# reset btn
# 1
resetBtn = tk.Button(btnFrame, text="RESET", font=(
    'Arial 10'), command=lambda: reset())
resetBtn.grid(row=0, column=0)
# take a pic btn
# 2
takePicBtn = tk.Button(btnFrame, text="TAKE A PIC!", font=(
    'Arial 16 bold'), command=lambda: setImagesAndAttr())
